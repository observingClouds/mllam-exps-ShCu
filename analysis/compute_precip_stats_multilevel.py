...
pred = xr.open_dataset("../evals/rain_mse_boxcox_unroll100.zarr", engine="zarr")
...
for level_idx, level in enumerate(levels):
    ...
    for i, start_time in enumerate(tqdm(start_times, desc=f"Processing start times (level={level})", leave=True)):
        for j, forecast_duration in enumerate(tqdm(forecast_durations, desc="Processing forecast durations", leave=False)):
            ...
            # Store counts
            n_t2m_minima[level_idx, i, j] = len(local_minima_t2m)
            n_rain_maxima[level_idx, i, j] = len(local_maxima_rain)
            n_matching[level_idx, i, j] = len(matching_cells)
            n_total_unique[level_idx, i, j] = total_unique

            # Calculate scores
            if len(local_minima_t2m) > 0:
                score_from_t2m_arr[level_idx, i, j] = len(matching_cells) / len(local_minima_t2m)

            if len(local_maxima_rain) > 0:
                score_from_rain_arr[level_idx, i, j] = len(matching_cells) / len(local_maxima_rain)

            # F1 score
            if score_from_t2m_arr[level_idx, i, j] + score_from_rain_arr[level_idx, i, j] > 0:
                f1_score_arr[level_idx, i, j] = 2 * (
                    score_from_t2m_arr[level_idx, i, j]
                    * score_from_rain_arr[level_idx, i, j]
                ) / (
                    score_from_t2m_arr[level_idx, i, j]
                    + score_from_rain_arr[level_idx, i, j]
                )
            ...
            # Jaccard similarity
            if total_unique > 0:
                jaccard_score_arr[level_idx, i, j] = len(matching_cells) / total_unique

            # Arithmetic mean
            mean_score_arr[level_idx, i, j] = (
                score_from_t2m_arr[level_idx, i, j]
                + score_from_rain_arr[level_idx, i, j]
            ) / 2
...
metrics_ds = xr.Dataset(
    {
        'n_t2m_minima': (['level', 'start_time', 'elapsed_forecast_duration'], n_t2m_minima),
        'n_rain_maxima': (['level', 'start_time', 'elapsed_forecast_duration'], n_rain_maxima),
        'n_matching': (['level', 'start_time', 'elapsed_forecast_duration'], n_matching),
        'n_total_unique': (['level', 'start_time', 'elapsed_forecast_duration'], n_total_unique),
        'score_from_t2m': (['level', 'start_time', 'elapsed_forecast_duration'], score_from_t2m_arr),
        'score_from_rain': (['level', 'start_time', 'elapsed_forecast_duration'], score_from_rain_arr),
        'f1_score': (['level', 'start_time', 'elapsed_forecast_duration'], f1_score_arr),
        'jaccard_score': (['level', 'start_time', 'elapsed_forecast_duration'], jaccard_score_arr),
        'mean_score': (['level', 'start_time', 'elapsed_forecast_duration'], mean_score_arr),
    },
    coords={
...
output_path = "../evals/precip_stats_multilevel_unroll100.nc"
